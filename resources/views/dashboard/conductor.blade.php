...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2>🚖 Conductor: {{ Auth::user()->nombre_completo }}</h2>
        <span class="badge bg-{{ $stats['disponible'] ? 'success' : 'secondary' }} fs-6">
            {{ $stats['disponible'] ? '🟢 Disponible' : '🔴 No disponible' }} ({{ $stats['estado'] }})
        </span>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="card text-white bg-success">
                <div class="card-body">
                    <h6>Viajes Hoy</h6>
                    <h3>{{ $stats['viajes_completados_hoy'] }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card text-white bg-primary">
                <div class="card-body">
                    <h6>Ingresos Hoy</h6>
                    <h3>${{ number_format($stats['ingresos_hoy'], 2) }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            ...
            <div class="row">
                ...
                <div class="col-md-4 text-end">
                    <form action="{{ url('viaje/'.$viajeActivo->id_viaje.'/finalizar') }}" method="POST" class="mb-2">
                        @csrf
                        <button type="submit" class="btn btn-success btn-lg">✓ Finalizar Viaje</button>
                    </form>
                    <form action="{{ url('viaje/'.$viajeActivo->id_viaje.'/cancelar') }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-outline-danger" onclick="return confirm('¿Cancelar?')">
                            ❌ Cancelar
                        </button>
                    </form>
                </div>
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-header bg-warning text-dark">
            <strong>📍 Solicitudes Disponibles ({{ $solicitudes->count() }})</strong>
        </div>
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-secondary">{{ $s->tarifa->tipo_servicio ?? '—' }}</span></td>
                            <td>
                        ...
                        @empty
                        <tr><td colspan="7" class="text-center text-muted py-3">No hay solicitudes en este momento</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-dark text-white">
            <strong>📋 Mi Historial de Hoy</strong>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
